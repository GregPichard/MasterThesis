...
# =============================================================================
MonthlyVariables_db = pd.read_csv('Monthly/IntlMonthlyVariables_db.csv', index_col = [0, 1], header = 0, parse_dates = True)
...
# Censoring extreme volatility values, at the 99.99% quantile
MonthlyAvailable_db.loc[list(MonthlyAvailable_db.Volatility > np.percentile(MonthlyAvailable_db.Volatility.dropna(), 99.99)), 'Volatility'] = None

# Dropping observations with stock price less than USD 1
MonthlyAvailable_db.loc[MonthlyAvailable_db.Close < 1, 'Close'] = None

# Amihud ratio components for model 2
MonthlyAvailable_db.loc[list(MonthlyAvailable_db.AmihudNumerator <= np.percentile(MonthlyAvailable_db.AmihudNumerator.dropna(), 0.01)) or list(MonthlyAvailable_db.AmihudNumerator > np.percentile(MonthlyAvailable_db.AmihudNumerator.dropna(), 99.99)), 'AmihudNumerator'] = None
MonthlyAvailable_db.loc[list(MonthlyAvailable_db.AmihudDenominator <= np.percentile(MonthlyAvailable_db.AmihudDenominator.dropna(), 0.01)) or list(MonthlyAvailable_db.AmihudDenominator > np.percentile(MonthlyAvailable_db.AmihudDenominator.dropna(), 99.99)), 'AmihudDenominator'] = None
...
# Issues with the book-to-market ratio : exclude negative values and those above the 99.99% quantile
MonthlyAvailable_db.loc[list(MonthlyAvailable_db.BookToMarketRatio <= 0) or list(MonthlyAvailable_db.BookToMarketRatio > np.percentile(MonthlyAvailable_db.BookToMarketRatio.dropna(), 99.99)), 'BookToMarketRatio'] = None
...
# Export
SumStats.to_latex('../SummaryStats/IntlSummaryTable.tex', header = ['N (obs.)', 'Mean', 'St. dev.', 'Min.', '25%', 'Median', '75%', 'Max.'])
# Correlation matrix
...
# Export
CorrMat.to_latex('../SummaryStats/IntlCorrTable.tex', index_names = True, header = NumberedHeader[:(len(NumberedHeader))])
...
del MonthlyAvailable_1lag_db, MonthlyAvailable_2lag_db, MonthlyAvailable_3lag_db, MonthlyAvailable_4lag_db
...
MonthlyAvailable_std_db = MonthlyAvailable_db[['Volatility', 'PctSharesHeldETF', 'PctSharesHeldOtherMutual']]
...
MonthlyAvailable_std_1lag_db = MonthlyAvailable_std_db.groupby(level = 0)[['PctSharesHeldETF', 'CompanyMarketCap', 'InvClose', 'AmihudRatio', 'PctBidAskSpread', 'BookToMarketRatio', 'RetPast12to1M', 'RetPast12to7M','GrossProfitability', 'Volatility', 'PctSharesHeldOtherMutual']].shift(1)
...
del MonthlyAvailable_std_1lag_db, MonthlyAvailable_std_2lag_db, MonthlyAvailable_std_3lag_db, MonthlyAvailable_std_4lag_db
...
QuarterlyVarianceRatio_panel = pd.read_csv('Quarterly/IntlStocks_5to1dVarianceRatio.csv', header = 0, parse_dates = True, skiprows = [0, 2])
QuarterlyVarianceRatio_panel.rename(index = str, columns = {'RIC':'Date'}, inplace = True)
...
del QuarterlyAvailable_1lag_db, QuarterlyAvailable_2lag_db, QuarterlyAvailable_3lag_db, QuarterlyAvailable_4lag_db

# Some infinite values have to be removed
QuarterlyAvailable_db.replace([np.inf, -np.inf], np.nan, inplace = True)

# Censoring the sample for extreme values of variance ratios
QuarterlyAvailable_db.loc[list(QuarterlyAvailable_db.VR > np.percentile(QuarterlyAvailable_db.VR.dropna(), 99.9)), 'VR'] = None
QuarterlyAvailable_db.loc[list(QuarterlyAvailable_db.absVR > np.percentile(QuarterlyAvailable_db.absVR.dropna(), 99.9)), 'absVR'] = None

# =============================================================================
# ## Regression estimates
# =============================================================================
# Model 1 : Volatility
# Full International (non-U.S.) sample, lagged controls
# Column "Baseline"
...
f = open('../Regression_Results/Intl/mod1_fit_All_Volatility.tex', 'w')
f.write(mod1_fit_All_Volatility.summary.as_latex())
f.close()

## Full International sample with a constant term, lagged controls and 4 volatility lags
...
#print(mod1_fit)
#f = open('../Regression_Results/Intl/mod1_All_Volatility_withLags.tex', 'w')
...
f = open('../Regression_Results/Intl/mod1_contemporaneousControls_fit.tex', 'w')
...
# Column "Inst. o\'ship controls" - Only mutual funds otherwise, the regressors matrix does not have full rank
mod1_All_Volatility_withLags_withFundcontrols = linearmodels.PanelOLS.from_formula('Volatility ~ 1 + PctSharesHeldETF_1lag + np.log(CompanyMarketCap_1lag) + InvClose_1lag + AmihudRatio_1lag + PctBidAskSpread_1lag + BookToMarketRatio_1lag + RetPast12to1M_1lag + GrossProfitability_1lag + PctSharesHeldOtherMutual_1lag + EntityEffects + TimeEffects + Volatility_1lag + Volatility_2lag + Volatility_3lag + Volatility_4lag', MonthlyAvailable_db)
...
f = open('../Regression_Results/Intl/mod1_fit_All_Volatility_withLags_withFundcontrols.tex', 'w')
...
f = open('../Regression_Results/Intl/mod1_fit_All_Volatility_std_withLags_withFundcontrols.tex', 'w')
...
#f = open('../Regression_Results/Intl/mod1_fit_All_diffVolatility_std_withLags_withFundcontrols.tex', 'w')
...
mod1_comp = linearmodels.panel.compare({'Baseline':mod1_fit_All_Volatility, 'Controls + Vol. lags': mod1_fit_All_Volatility_withLags_Controls, 'Inst. o\'ship control':mod1_fit_All_Volatility_withLags_withFundcontrols , 'Standardized': mod1_fit_All_Volatility_std_withLags_withFundcontrols})
print(mod1_comp)
f = open('../Regression_Results/Intl/mod1_comp.tex', 'w')
...
f = open('../Regression_Results/Intl/mod2_fit_All_Liquidity_BidAsk.tex', 'w')
f.write(mod2_fit_All_Liquidity_BidAsk.summary.as_latex())
f.close()

mod2_All_Liquidity_BidAsk_withFundcontrols = linearmodels.PanelOLS.from_formula('PctBidAskSpread ~ 1 + PctSharesHeldETF_1lag + np.log(CompanyMarketCap_1lag) + BookToMarketRatio_1lag + Volatility_1lag + EntityEffects + TimeEffects + PctSharesHeldOtherMutual_1lag + PctSharesHeldPension_1lag ', MonthlyAvailable_db)
...
f = open('../Regression_Results/Intl/mod2_fit_All_Liquidity_BidAsk_withFundcontrols.tex', 'w')
f.write(mod2_fit_All_Liquidity_BidAsk_withFundcontrols.summary.as_latex())
f.close()

# Amihud ratio decomposition
mod2_All_Liquidity_Amihud = linearmodels.PanelOLS.from_formula('AmihudNumerator ~ 1 + PctSharesHeldETF_1lag + np.log(CompanyMarketCap_1lag) + BookToMarketRatio_1lag + np.log(AmihudDenominator) + EntityEffects + TimeEffects', MonthlyAvailable_db)
mod2_fit_All_Liquidity_Amihud = mod2_All_Liquidity_Amihud.fit(cov_type = "kernel")
print(mod2_fit_All_Liquidity_Amihud)
f = open('../Regression_Results/Intl/mod2_fit_All_Liquidity_Amihud.tex', 'w')
f.write(mod2_fit_All_Liquidity_Amihud.summary.as_latex())
f.close()

mod2_All_Liquidity_Amihud_withFundcontrols = linearmodels.PanelOLS.from_formula('AmihudNumerator ~ 1 + PctSharesHeldETF_1lag + np.log(CompanyMarketCap_1lag) + BookToMarketRatio_1lag + np.log(AmihudDenominator) + EntityEffects + TimeEffects + PctSharesHeldOtherMutual_1lag ', MonthlyAvailable_db)
...
f = open('../Regression_Results/Intl/mod2_fit_All_Liquidity_Amihud_withFundcontrols.tex', 'w')
...
print(mod2_comp)
f = open('../Regression_Results/Intl/mod2_comp.tex', 'w')
...
f = open('../Regression_Results/Intl/mod3_fit_All_absVR.tex', 'w')
...
f = open('../Regression_Results/Intl/mod3_fit_All_absVR_withFundcontrols.tex', 'w')
...
print(mod3_comp)
f = open('../Regression_Results/Intl/mod3_comp.tex', 'w')
